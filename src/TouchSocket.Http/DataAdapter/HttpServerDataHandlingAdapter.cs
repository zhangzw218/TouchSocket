...
internal sealed class HttpServerDataHandlingAdapter : SingleStreamDataHandlingAdapter
{
        ...
        while (reader.BytesRemaining > 0)
        {
            ...
            if (this.m_currentRequest == null)
            {
                ...
                if (this.m_currentRequest.ParsingHeader(ref reader))
                {
                    ...
                    else if (this.m_currentRequest.ContentLength > reader.BytesRemaining)
                    {
    ...
    {
        // 查找 CRLF
        var crlfIndex = this.FindCRLF(reader);
        if (crlfIndex < 0)
        {
        ...
        if (!this.TryParseChunkSize(chunkSizeSpan, out var chunkSize))
        {
    ...
    private int FindCRLF<TReader>(TReader reader) where TReader : IBytesReaderV4
    {
    ...
    private bool TryParseChunkSize(ReadOnlySpan<byte> hexSpan, out long chunkSize)
    {
        chunkSize = 0;
    ...
    private bool TrySkipCRLF<TReader>(TReader reader) where TReader : IBytesReaderV4
    {
        if (reader.BytesRemaining >= 2)
        {
            var span = reader.GetSpan(2);
