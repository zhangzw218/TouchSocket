...
public interface IPluginManager : IDisposableObject, IResolverObjectV4
{
    ...
    void Add<[DynamicallyAccessedMembers(AOT.PluginMemberType)] TPlugin>(TPlugin plugin) where TPlugin : class, IPlugin;
    ...
    IPlugin Add([DynamicallyAccessedMembers(AOT.PluginMemberType)] Type pluginType);
    ...
    void Add([DynamicallyAccessedMembers(AOT.PluginMemberType)] Type pluginType, Func<object, PluginEventArgs, Task> pluginInvokeHandler, Delegate sourceDelegate = default);
    ...
    ValueTask<bool> RaiseAsync([DynamicallyAccessedMembers(AOT.PluginMemberType)] Type pluginType, IResolver resolver, object sender, PluginEventArgs e);
    ...
    void Remove([DynamicallyAccessedMembers(AOT.PluginMemberType)] Type pluginType, Delegate func);
}