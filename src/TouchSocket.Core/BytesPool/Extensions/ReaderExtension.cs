...
/// <summary>
/// 为<see cref="IBytesReaderV4"/>提供扩展方法的静态类，用于读取各种类型的数据。
/// </summary>
/// <remarks>
/// 此类提供了丰富的扩展方法，支持读取基本数据类型、字符串、字节块、枚举等。
/// 所有方法都是针对实现了<see cref="IBytesReaderV4"/>接口的类型进行扩展。
/// </remarks>
public static partial class ReaderExtension
{
    ...
    /// <summary>
    /// 从字节读取器中读取一个<see cref="ByteBlockV4"/>对象。
    /// </summary>
    /// <typeparam name="TReader">实现<see cref="IBytesReaderV4"/>接口的读取器类型。</typeparam>
    /// <param name="reader">字节读取器实例。</param>
    /// <returns>读取的<see cref="ByteBlockV4"/>对象，如果数据为空则返回默认值。</returns>
    /// <remarks>
    /// 此方法首先读取长度信息，然后根据长度创建<see cref="ByteBlockV4"/>并填充数据。
    /// </remarks>
    public static ByteBlockV4 ReadByteBlock<TReader>(ref TReader reader)
        where TReader : IBytesReaderV4
    {
        ...
        var byteBlock = new ByteBlockV4(len);
