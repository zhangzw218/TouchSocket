...
public static partial class ReaderExtension
{
    ...
    /// </summary>
    /// <typeparam name="TReader">实现<see cref="IBytesReaderV4"/>接口的字节读取器类型，必须为引用类型。</typeparam>
    ...
    /// <typeparam name="TReader">实现<see cref="IBytesReaderV4"/>接口的字节读取器类型，必须为引用类型。</typeparam>
    /// <param name="reader">要读取数据的字节读取器。</param>
    /// <returns>读取的<see cref="ByteBlockV4"/>对象，如果长度小于0则返回默认值。</returns>
    /// <remarks>
    /// 此方法首先读取可变长度的32位无符号整数作为长度信息，然后读取相应长度的数据创建字节块。
    /// 创建的字节块会自动设置到起始位置。
    /// </remarks>
    public static ByteBlockV4 ReadByteBlock<TReader>(this TReader reader)
        where TReader : class, IBytesReaderV4
    {
