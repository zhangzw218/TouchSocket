...
/// 所有方法都是针对实现了<see cref="IBytesWriterV4"/>接口的类型进行扩展。
/// </remarks>
public static partial class WriterExtension
{
    ...
    /// <summary>
    /// 将<see cref="ByteBlockV4"/>对象写入字节写入器。
    /// </summary>
    /// <typeparam name="TWriter">实现<see cref="IBytesWriterV4"/>接口的写入器类型。</typeparam>
    /// <param name="writer">字节写入器实例。</param>
    /// <param name="byteBlock">要写入的<see cref="ByteBlockV4"/>对象。</param>
    /// <remarks>
    /// 如果<paramref name="byteBlock"/>为 <see langword="null"/>，则写入长度为0；
    /// 否则先写入长度信息（长度+1），再写入字节块内容。
    /// </remarks>
    public static void WriteByteBlock<TWriter>(ref TWriter writer, ByteBlockV4 byteBlock)
        where TWriter : IBytesWriterV4
    {
