...
public static class MqttExtension
{
    ...
    /// </summary>
    /// <typeparam name="TReader">字节读取器的类型，必须实现<see cref="IBytesReaderV4"/>接口。</typeparam>
    ...
    /// 将MQTT固定头部写入字节写入器。
    /// </summary>
    /// <typeparam name="TWriter">字节写入器的类型，必须实现<see cref="IBytesWriterV4"/>接口。</typeparam>
    ...
    /// </summary>
    /// <typeparam name="TWriter">字节写入器类型，需实现<see cref="IBytesWriterV4"/>接口。</typeparam>
    /// <param name="writer">要写入的字节写入器。</param>
    ...
    /// </remarks>
    public static ReadOnlyMemory<byte> ReadMqttBinaryData<TReader>(ref TReader reader) where TReader : IBytesReaderV4
    {
        var length = ReaderExtension.ReadValue<TReader, ushort>(ref reader, EndianType.Big);
    ...
    /// </remarks>
    public static void WriteUserProperties<TWriter>(ref TWriter writer, IReadOnlyList<MqttUserProperty> userProperties) where TWriter : IBytesWriterV4
